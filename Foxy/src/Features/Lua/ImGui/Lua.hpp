...
namespace sol_ImGui
{
	...
	inline int GetKeyPressedAmount(int imgui_key, float repeat_delay, float rate) { return ImGui::GetKeyPressedAmount(static_cast<ImGuiKey>(imgui_key), repeat_delay, rate); }
...
#pragma endregion MouseCursor
	...
	inline void InitUserTypes(sol::state& lua) {	
		lua.new_usertype<ImGuiIO>(
			"ImGuiIO", sol::default_constructor,
		...
		lua.new_usertype<ImColor>(
			"ImColor", sol::constructors<ImColor(), ImColor(int, int, int, int), ImColor(ImU32), ImColor(float, float, float, float), ImColor(const ImVec4&)>(),
			"Value", &ImColor::Value
		);
		...
		lua.new_usertype<ImGuiStyle>(
			"ImGuiStyle", sol::default_constructor,
...
#pragma region Color Utilities
		...
		ImGui.set_function("ColorConvertRGBAToU32", [=](Color col) { return IM_COL32(col.r, col.g, col.b, col.a); });
