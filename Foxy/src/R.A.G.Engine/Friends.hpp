...
static_assert(sizeof(CFriendInfo) == 0x180);
class CFriendRegistry {
public:
	uint32_t m_friend_count; //0x0000
	char pad_0004[8]; //0x0004
	CFriendInfo(*m_friends)[250]; //0x000C

	CFriendInfo* get(uint32_t idx) {
		return &(*m_friends)[idx];
	}
}; //Size: 0x0014
static_assert(sizeof(CFriendRegistry) == 0x14);
#pragma pack(pop)